...
class Joueur {
	...
	public boolean shoot(int i) throws RaiseException {
		...
		if (this.score.size() < 9) {
			boolean ret = normalShoot(i);
			return ret;
		}

		//Round 10
		if (this.score.size() == 9) {
			boolean ret=shootNumber10(i);
			return ret;
		}

		//supplementary round
		if (this.score.size() == 10) {
			boolean ret=shootNumber11(i);
			return ret;
		}
		return false;
	}
	...
	private boolean normalShoot(int i) throws RaiseException {
	...
	private boolean shootNumber10(int i) throws RaiseException {
		if (expectedThrow2 == false)
		{
			if (i == 10) //strike
			{
	...
	private boolean shootNumber11(int i) throws  RaiseException {
